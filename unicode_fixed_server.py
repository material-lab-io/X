#!/usr/bin/env python3
"""
Unicode Fixed Flask Server - Properly handles emojis and special characters
"""
...
def fix_unicode_escapes(text):
    ...
    # Fix double-escaped sequences (\\uXXXX -> \uXXXX)
    if '\\\\u' in text:
    ...
    try:
        ...
        # If decoding fails, try a different approach
        try:
            # Handle raw unicode escape sequences
        ...
        if Path('generated_threads_final.json').exists():
            ...
            for item in (data if isinstance(data, list) else [data]):
                ...
                for i, tweet in enumerate(raw_tweets):
                    ...
                    # Fix Unicode escapes
                    tweet_text = fix_unicode_escapes(tweet_text)
...
if __name__ == '__main__':
    port = 5000
    print(f"\n🚀 Unicode Fixed Tweet Generator Server")
    print(f"📍 URL: http://localhost:{port}/")
    print(f"\nThis server properly handles emojis and Unicode characters!")
