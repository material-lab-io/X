#!/usr/bin/env python3
"""
Safe Flask Server - Handles Unicode and all edge cases properly
"""
...
# HTML Template with proper Unicode handling
HTML_TEMPLATE = """
<!DOCTYPE html>
<html>
<head>
...
def safe_unicode_decode(text):
    ...
    # Already decoded emoji, return as-is
    if any(ord(c) > 127 for c in text):
        return text
    
    # Try to decode unicode escapes
    try:
        # Handle Python unicode escapes like \ud83d\udc47
        if '\\u' in text:
            # This is safer than encode().decode('unicode-escape')
            import codecs
...
def generate_with_api(topic, content_type, template, context):
    ...
    try:
        ...
        # Correct content type values for UnifiedTweetGenerator
        ...
        if Path('generated_threads_final.json').exists():
            ...
            for item in (data if isinstance(data, list) else [data]):
                ...
                for i, tweet in enumerate(raw_tweets):
                    ...
                    tweet_text = safe_unicode_decode(tweet_text)
                    
                    # Calculate real character count (Twitter counts emojis properly)
...
if __name__ == '__main__':
    port = 5000
    print(f"\n🚀 Safe Tweet Generator Server")
    print(f"📍 URL: http://localhost:{port}/")
    print(f"\nThis server safely handles Unicode and emojis!")
