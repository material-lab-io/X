#!/usr/bin/env python3
"""
Final Flask Server - Simple, robust, and handles Unicode properly
"""
                    ...
                    {% for thread in threads %}
                        {% for tweet in thread.tweets %}
                            <div class="tweet">
                                ...
                                <div class="tweet-content">{{ tweet.text|safe }}</div>
...
def fix_unicode_in_text(text):
    ...
    # Handle the Unicode properly by decoding the escape sequences
    try:
        # First, check if we have raw escape sequences
        if '\\u' in text:
...
def generate_with_api(topic, content_type, template, context):
    ...
    try:
        ...
        import sys
        sys.path.insert(0, '/home/kushagra/X/XPosts')
    ...
    if request.method == 'POST':
        ...
        if form_data['topic'] and form_data['template']:
            ...
            try:
                ...
                with open('generated_threads_final.json', 'w', encoding='utf-8') as f:
        ...
        if Path('generated_threads_final.json').exists():
            ...
            for item in (data if isinstance(data, list) else [data]):
                ...
                for i, tweet in enumerate(raw_tweets):
                    ...
                    # Fix Unicode escapes
                    tweet_text = fix_unicode_in_text(tweet_text)
...
def regenerate():
    if current_generation:
        # Simulate a new POST request to /generate
...
if __name__ == '__main__':
    port = 5000
    print(f"\n🚀 Final Tweet Generator Server")
    print(f"📍 URL: http://localhost:{port}/")
    print(f"\nThis server handles all edge cases properly!")
